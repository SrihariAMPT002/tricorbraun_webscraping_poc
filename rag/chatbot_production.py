...
3. AGGREGATION_QUERY: Statistics/calculations OR finding products by extreme values
   ...
   Keywords: average, mean, count, total, sum, max, min, largest, smallest, biggest, which product, what product
...
class GuardrailSystem:
    ...
    def check_query_scope(self, query: str, query_analysis: Dict) -> Tuple[bool, str]:
        product_keywords = [
            ...
            'boston round', 'wine', 'liquor', 'ml', 'oz', 'price', 'capacity', 'product',
            'largest', 'smallest', 'average', 'count', 'how many'
        ]
...
class HybridRAGChatbot:
    ...
    def __init__(self, verbose: bool = False, log_file: str = "chatbot_activity.log"):
        ...
        self.logger.console_logger.info("🚀 Initializing Production-Ready Hybrid RAG Chatbot...")
        ...
        self.logger.console_logger.info(f"📊 Using capacity field: {self.CAPACITY_FIELD}")
    ...
    def aggregation_query_mongodb(self, filters: Dict, aggregation_type: str, 
                                   aggregation_field: str, return_products: bool = False) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            aggregation_field: Field to aggregate on ("capacity" or "price")
        ...
        # For statistical aggregations (avg, count, sum)
        else:
            if match_stage:
                pipeline.append({"$match": match_stage})
            ...
            if aggregation_type == "count":
                ...
                group_field = self.COLOR_FIELD if aggregation_field == "capacity" else actual_field
                pipeline.append({
                    "$group": {
                        "_id": f"${group_field}",
                        "count": {"$sum": 1},
                        "avg_capacity": {"$avg": f"${self.CAPACITY_FIELD}"}
                    }
                })
    ...
    def _build_mongodb_query(self, filters: Dict) -> Dict:
        ...
        if filters.get("color"):
            mongo_query[self.COLOR_FIELD] = {"$regex": filters["color"], "$options": "i"}
        if filters.get("material"):
            mongo_query[self.MATERIAL_FIELD] = {"$regex": filters["material"], "$options": "i"}
        if filters.get("shape"):
            mongo_query[self.SHAPE_FIELD] = {"$regex": filters["shape"], "$options": "i"}
        if filters.get("company"):
            mongo_query[self.COMPANY_FIELD] = {"$regex": filters["company"], "$options": "i"}
        ...
        if filters.get("capacity_range"):
            ...
            if cap_range.get("min") or cap_range.get("max"):
                ...
                if cap_range.get("min"):
                    mongo_query[self.CAPACITY_FIELD]["$gte"] = cap_range["min"]
                if cap_range.get("max"):
                    mongo_query[self.CAPACITY_FIELD]["$lte"] = cap_range["max"]
    ...
    def format_aggregation_context(self, aggregation_result: Dict, filters: Dict) -> str:
        ...
        if return_products:
            ...
            if agg_type == "count":
                ...
                for item in results:
                    context += f"- {item.get('_id')}: {item.get('count')} products (avg capacity: {item.get('avg_capacity', 'N/A')} ml)\n"
    ...
    def query(self, user_query: str) -> str:
        ...
        try:
            ...
            if query_type == QueryType.AGGREGATION_QUERY:
                ...
                agg_field = analysis.get("aggregation_field", "capacity")
                return_products = analysis.get("return_products", False)

                agg_result = self.aggregation_query_mongodb(
                    filters, agg_type, agg_field, return_products
                )
...
def main():
    print("="*100)
    print("PRODUCTION-READY CHATBOT - TAILORED TO YOUR MONGODB SCHEMA")
    print("="*100)
    ...
    test_queries = [
        ...
        "Show me Boston Round bottles under $2",
    ]
