...
# Create a fixed version with better query routing logic
...
1. SEMANTIC_SEARCH: User describes products conceptually or asks about product types
   Examples: "bottles for essential oils", "elegant wine bottles", "amber bottles", "Boston round bottles"
   → Requires vector similarity search in Pinecone

2. STRUCTURED_QUERY: User has ONLY specific numerical/categorical filters, NO semantic description
   Examples: "500ml bottles under $2", "bottles between 100ml and 200ml"
   → Requires MongoDB structured query ONLY

3. HYBRID: Query has BOTH semantic description AND specific filters
   Examples: "modern looking bottles in 750ml size", "elegant amber bottles under $5"
   → Requires both Pinecone vector search + MongoDB filtering

4. OUT_OF_SCOPE: Query is CLEARLY not about searching for glass bottles/jars
...
{{
  ...
  "is_data_related": boolean (TRUE for any product/bottle/jar related query)
}}
...
"""
        ...
        try:
            ...
            # Default to HYBRID if classification is uncertain for product queries
            query_type_str = analysis.get("query_type", "HYBRID")
            
            # Override: If is_data_related is True but query_type is OUT_OF_SCOPE, default to SEMANTIC_SEARCH
            if analysis.get("is_data_related", True) and query_type_str == "OUT_OF_SCOPE":
                query_type_str = "SEMANTIC_SEARCH"
                self.logger.console_logger.info("Overriding OUT_OF_SCOPE to SEMANTIC_SEARCH (data-related query)")
...
class GuardrailSystem:
    ...
    def check_query_scope(self, query: str, query_analysis: Dict) -> Tuple[bool, str]:
        ...
        # IMPORTANT: Be permissive - only reject clearly out-of-scope queries
        is_data_related = query_analysis.get("is_data_related", True)
        ...
        # Only reject if explicitly marked as out of scope AND no product keywords
        if query_type == "OUT_OF_SCOPE" and not has_product_keyword:
            return False, "I'm a product catalog assistant specialized in glass bottles and jars. I can help you find products, check pricing, verify availability, and compare options. What would you like to know about our bottle and jar catalog?"
    ...
    def validate_response(self, response: str, context: str) -> Tuple[bool, List[str]]:
        ...
        # If no context (no results), skip validation
...
3. If information is not in context, explicitly state "This information is not available in our current results"
...
Provide an accurate response based ONLY on the above product information."""
    ...
    def query(self, user_query: str) -> str:
        ...
        try:
            ...
            # Step 2: Guardrail check (more permissive now)
...
def main():
    print("="*100)
    print("ENHANCED CHATBOT WITH QUERY ROUTING, LOGGING & GUARDRAILS (FIXED)")
    print("="*100)
...
with open('enhanced_chatbot_fixed.py', 'w') as f:
    f.write(fixed_code)

print("✅ Fixed version created: enhanced_chatbot_fixed.py")
print("\n🔧 Key fixes:")
print("1. More permissive scope checking - only rejects clearly non-product queries")
print("2. Product keyword detection to catch bottle/jar queries")
print("3. Override logic: if query is data-related, don't mark as OUT_OF_SCOPE")
print("4. Default to HYBRID for uncertain cases (safer)")
print("5. Better JSON parsing with markdown cleanup")
print("6. Skip response validation if no results found")
